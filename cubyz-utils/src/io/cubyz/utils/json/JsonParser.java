...
public abstract class JsonParser {
	public static JsonObject parseObjectFromFile(String path) {
		...
		if(characters[0] != '{') {
			Log.warning("Expected the json file to start with an object: "+path);
		}
		int[] index = new int[] {1};
	...
	private static void printError(int index, char[] chars, String message) {
		...
		Log.warning("Syntax error reading json file \"" + filePathForErrorHandling + "\" in line " + line + ": " + message);
	}
