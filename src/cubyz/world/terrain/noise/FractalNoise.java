...
public class FractalNoise {
	private static long getSeed(int x, int z, int offsetX, int offsetZ, int worldSizeX, int worldSizeZ, long seed, int scale, int maxResolution) {
		Random rand = new Random(seed*(scale*maxResolution | 1));
		long l1 = rand.nextLong() | 1;
		long l2 = rand.nextLong() | 1;
		return (((long)(World.worldModulo((offsetX + x)*maxResolution, worldSizeX)))*l1) ^ seed ^ (((long)(World.worldModulo((offsetZ + z)*maxResolution, worldSizeZ)))*l2);
	}
	public static void generateFractalTerrain(int wx, int wz, int x0, int z0, int width, int height, int scale, long seed, int worldSizeX, int worldSizeZ, float[][] map, int maxResolution) {
		int max =scale+1;
		int and = scale-1;
	...
	public static void generateInitializedFractalTerrain(int offsetX, int offsetZ, int scale, int startingScale, long seed, int worldSizeX, int worldSizeZ, float[][] bigMap, float lowerLimit, float upperLimit, int maxResolution) {
		...
		for(int res = startingScale*2; res != 0; res >>>= 1) {
			...
			for(int x = 0; x < max; x += res<<1) {
				for(int y = res; y+res < max; y += res<<1) {
					rand.setSeed(getSeed(x, y, offsetX, offsetZ, worldSizeX, worldSizeZ, seed, res, maxResolution));
	 ...
	 * @param maxResolution
	 */
	public static void generateSparseFractalTerrain(int wx, int wz, int width, int height, int scale, long seed, int worldSizeX, int worldSizeZ, float[][] map, int maxResolution) {
		wx /= maxResolution;
		wz /= maxResolution;
		width /= maxResolution;
		...
		for(int x0 = 0; x0 < width; x0 += scale) {
			for(int y0 = 0; y0 < height; y0 += scale) {
				generateFractalTerrain(wx + x0, wz + y0, x0, y0, Math.min(width-x0, scale), Math.min(height-y0, scale), scale, seed, worldSizeX, worldSizeZ, map, maxResolution);
			}
		}
	}
	/**
	 ...
	 * @return
	 */
	public static float[][] generateFractalTerrain(int wx, int wz, int width, int height, int scale, long seed, int worldSizeX, int worldSizeZ) {
		float[][] map = new float[width][height];
		for(int x0 = 0; x0 < width; x0 += scale) {
			for(int z0 = 0; z0 < height; z0 += scale) {
				generateFractalTerrain(wx + x0, wz + z0, x0, z0, Math.min(width-x0, scale), Math.min(height-z0, scale), scale, seed, worldSizeX, worldSizeZ, map, 1);
			}
		}
		return map;
	}
