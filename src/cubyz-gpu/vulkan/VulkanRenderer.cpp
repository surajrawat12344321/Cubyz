...
public:
	...
	void populateDebugMessengerCreateInfo(VkDebugUtilsMessengerCreateInfoEXT& createInfo) {
		...
		// Not using VK_DEBUG_UTILS_MESSAGE_SEVERITY_INFO_BIT_EXT here because it spams the log file.
	...
	void setupDebugMessenger() {
		...
		if (result != VK_SUCCESS) {
			throw std::runtime_error("Failed to set up debug messenger! Error code: "+std::to_string(result));
		}
	}
	...
	void pickPhysicalDevice() {
		...
		// Use the most suitable device from the list:
	...
	void createFramebuffers() {
		...
		for (size_t i = 0; i < swapChainImageViews.size(); i++) {
			...
			if (result != VK_SUCCESS) {
				throw std::runtime_error("Failed to create framebuffer! Error code: "+std::to_string(result));
			}
		}
	}
	...
	void createCommandPool() {
		...
		if (result != VK_SUCCESS) {
			throw std::runtime_error("Failed to create graphics command pool! Error code: "+std::to_string(result));
		}
	}
	...
	void createTextureImage() {
		...
		stbi_uc* pixels = stbi_load("assets/textures/test.png", &texWidth, &texHeight, &texChannels, STBI_rgb_alpha);
	...
	VkImageView createImageView(VkImage image, VkFormat format, VkImageAspectFlags aspectFlags) {
		...
		if (result != VK_SUCCESS) {
			throw std::runtime_error("Failed to create texture image view! Error code: "+std::to_string(result));
		}
	...
	void createImage(uint32_t width, uint32_t height, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage& image, VkDeviceMemory& imageMemory) {
		...
		if (result != VK_SUCCESS) {
			throw std::runtime_error("Failed to allocate image memory! Error code: "+std::to_string(result));
		}
	...
	void createCommandBuffers() {
		...
		for (size_t i = 0; i < commandBuffers.size(); i++) {
			...
			if (result != VK_SUCCESS) {
				throw std::runtime_error("Failed to begin recording command buffer! Error code: "+std::to_string(result));
			}
			...
			if (result != VK_SUCCESS) {
				throw std::runtime_error("Failed to record command buffer! Error code: "+std::to_string(result));
			}
		}
	}
	...
	void drawFrame() {
		...
		if (result == VK_ERROR_OUT_OF_DATE_KHR) {
			...
			throw std::runtime_error("failed to acquire swap chain image! Error code: "+std::to_string(result));
		}
		...
		if (result != VK_SUCCESS) {
			throw std::runtime_error("Failed to submit draw command buffer! Error code: "+std::to_string(result));
		}
	...
	VkPresentModeKHR chooseSwapPresentMode(const std::vector<VkPresentModeKHR>& availablePresentModes) {
		for (const auto& availablePresentMode : availablePresentModes) {
			if (availablePresentMode == VK_PRESENT_MODE_MAILBOX_KHR) {
				return availablePresentMode; // TODO: Check if the latency of this triple buffering is visible to the user.
			}
		}
	...
	// Checks if a device is suitable and returns a score. Returns 0 if the device isn't suitable.
	int isDeviceSuitable(VkPhysicalDevice device) {
		...
		// Check swap chain and extension support:
	...
	static VKAPI_ATTR VkBool32 VKAPI_CALL debugCallback(VkDebugUtilsMessageSeverityFlagBitsEXT messageSeverity, VkDebugUtilsMessageTypeFlagsEXT messageType, const VkDebugUtilsMessengerCallbackDataEXT* pCallbackData, void* pUserData) {
		switch(messageSeverity) {
			case VK_DEBUG_UTILS_MESSAGE_SEVERITY_VERBOSE_BIT_EXT:
				logger::log(pCallbackData->pMessage, "VK_VERBOSE", logger::DEBUG);
				break;
			case VK_DEBUG_UTILS_MESSAGE_SEVERITY_INFO_BIT_EXT:
				logger::log(pCallbackData->pMessage, "VK_INFO", logger::INFO);
				break;
			case VK_DEBUG_UTILS_MESSAGE_SEVERITY_WARNING_BIT_EXT:
				logger::log(pCallbackData->pMessage, "VK_WARNING", logger::WARNING);
				break;
			case VK_DEBUG_UTILS_MESSAGE_SEVERITY_ERROR_BIT_EXT:
				logger::log(pCallbackData->pMessage, "VK_ERROR", logger::ERROR);
				break;
			default:
				logger::log(pCallbackData->pMessage, "VK_UNKNOWN("+std::to_string(VK_DEBUG_UTILS_MESSAGE_SEVERITY_VERBOSE_BIT_EXT)+")", logger::INFO);
				break;
		}
